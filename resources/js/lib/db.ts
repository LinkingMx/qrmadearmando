    ...
    upgrade(db, oldVersion, newVersion, transaction) {
      ...
      if (oldVersion < 1) {
        ...
        if (!db.objectStoreNames.contains('gift_cards')) {
          ...
          gcStore.createIndex('by-legacy-id', 'legacy_id', { unique: false })
...
export async function queueOfflineAction(action: Omit<OfflineAction, 'id'>): Promise<string> {
  const db = await initDB()
...
export async function updateOfflineActionError(id: string, error: string): Promise<void> {
  const db = await initDB()
